...
interface Book {
    ...
    autor: string;
    editor: string;
    estadoConservacao: string; // 🔥 Corrigido para coincidir com o backend
    usuario: { // 🔥 Agora `usuario` é um objeto, e não apenas um ID
      id: number;
      nome: string;
    };
  }
            ...
            <div key={book.id} className="vintage-card overflow-hidden rounded-lg">
              <div className="p-6">
                ...
                <p className="text-[#594a42] font-serif text-sm mt-1">
                  Estado: <span className="font-bold">{book.estadoConservacao || 'Não informado'}</span>
                </p>
  
                {/* Exibe o nome e ID do dono do livro */}
                <div className="mt-2 text-sm text-gray-700">
                  <strong>Dono do livro:</strong> {book.usuario?.nome} (ID: {book.usuario?.id})
                </div>
